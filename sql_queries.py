...
songplay_table_create = ("""CREATE TABLE IF NOT EXISTS songplays (songplay_id varchar, start_time varchar, user_id varchar, level varchar, song_id varchar, artist_id varchar, session_id varchar, location varchar, user_agent varchar, PRIMARY KEY (songplay_id));""")
...
time_table_create = ("""CREATE TABLE IF NOT EXISTS time (time_id timestamp, start_time time, hour int, day int, week int, month int, year int, weekday int, PRIMARY KEY (time_id));
""")
...
time_table_insert = ("""INSERT INTO time (time_id, start_time, hour, day, week, month, year, weekday) VALUES(%s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT (time_id) DO NOTHING;""")

song_select = ("""SELECT songs.song_id, artists.artist_id from artists JOIN songs ON artists.artist_id = songs.artist_id WHERE songs.title = %s AND artists.name = %s OR songs.duration = %s""")
